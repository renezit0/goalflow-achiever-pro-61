...
export default function MetasLojaPage() {
  ...
  useEffect(() => {
    if (user && periodo && currentLojaId) {
      fetchMetas();
    }
  }, [user, periodo, currentLojaId]);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
              {metas.map((meta, index) => (
                <Card 
                  key={index} 
                  className={`
                    relative overflow-hidden border-0 shadow-lg bg-gradient-to-br from-background to-muted/20
                    hover:shadow-xl hover:scale-[1.02] transition-all duration-300 ease-out
                    ${meta.progresso >= 100 ? 'ring-2 ring-success/30 bg-gradient-to-br from-success/5 to-success/10' : 
                      meta.progresso >= 80 ? 'ring-2 ring-warning/30 bg-gradient-to-br from-warning/5 to-warning/10' : 
